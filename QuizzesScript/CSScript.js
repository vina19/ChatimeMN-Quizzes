...
function setQuestion(qCount, rand) {
    let ques = CustomerServiceQuestions[rand];
...
function randomGenerator() {
	while(status == 0) {
		rand = Math.round(Math.random() * CustomerServiceQuestions.length);
		if(rand !== CustomerServiceQuestions.length) {
...
function startTimer(secs, elem) {
	...
	if(secs<0) {
		...
		if(button1.style.backgroundColor !== "rgb(26, 255, 26)" && button2.style.backgroundColor !== "rgb(26, 255, 26)" && button3.style.backgroundColor !== "rgb(26, 255, 26)" && button4.style.backgroundColor !== "rgb(26, 255, 26)") {
			...
			if(questionCount == 9) {
				if(ans === CustomerServiceQuestions[rand].answer) {
					setCorrect();
				}
				else {
					setWrong();
...
rand = Math.round(Math.random() * CustomerServiceQuestions.length);
while(rand == CustomerServiceQuestions.length) {
	rand = Math.round(Math.random() * CustomerServiceQuestions.length);
}
