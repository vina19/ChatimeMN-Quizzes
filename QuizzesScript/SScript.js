...
function setQuestion(qCount, rand) {
    let ques = SmoothieQuestions[rand];
...
function randomGenerator() {
	while(status == 0) {
		rand = Math.round(Math.random() * SmoothieQuestions.length);
		if(rand !== SmoothieQuestions.length) {
...
function startTimer(secs, elem) {
	...
	if(secs<0) {
		...
		if(button1.style.backgroundColor !== "rgb(26, 255, 26)" && button2.style.backgroundColor !== "rgb(26, 255, 26)" && button3.style.backgroundColor !== "rgb(26, 255, 26)" && button4.style.backgroundColor !== "rgb(26, 255, 26)") {
			...
			if(questionCount == 9) {
				if(ans === SmoothieQuestions[rand].answer) {
					setCorrect();
				}
				else {
					setWrong();
...
rand = Math.round(Math.random() * SmoothieQuestions.length);
while(rand == SmoothieQuestions.length) {
	rand = Math.round(Math.random() * SmoothieQuestions.length);
}
