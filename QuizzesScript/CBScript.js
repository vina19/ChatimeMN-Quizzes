...
function setQuestion(qCount, rand) {
    let ques = CremeBruleeQuestions[rand];
...
function randomGenerator() {
	while(status == 0) {
		rand = Math.round(Math.random() * CremeBruleeQuestions.length);
		if(rand !== CremeBruleeQuestions.length) {
...
function startTimer(secs, elem) {
	...
	if(secs<0) {
		...
		if(button1.style.backgroundColor !== "rgb(26, 255, 26)" && button2.style.backgroundColor !== "rgb(26, 255, 26)" && button3.style.backgroundColor !== "rgb(26, 255, 26)" && button4.style.backgroundColor !== "rgb(26, 255, 26)") {
			...
			if(questionCount == 9) {
				if(ans === CremeBruleeQuestions[rand].answer) {
					setCorrect();
				}
				else {
					setWrong();
...
rand = Math.round(Math.random() * CremeBruleeQuestions.length);
while(rand == CremeBruleeQuestions.length) {
	rand = Math.round(Math.random() * CremeBruleeQuestions.length);
}
